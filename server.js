...
app.post("/chat", async (req, res) => {
    try {
        ...
        // Full agentic Amex system prompt
        const systemPrompt = `
...
`;
        ...
        const reply = gptResponse.choices?.[0]?.message?.content || "Sorry, I couldn't process your request.";
        res.json({ reply });

    } catch (error) {
        console.error(error);
        res.status(500).json({ error: "Error talking to GPT" });
    }
});
